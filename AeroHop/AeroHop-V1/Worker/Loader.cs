...
public class Loader
{
    ...
    public string PackageManager()
    {
        ...
        Pm[] packageManagers = new[] { Pm.apt, Pm.dnf, Pm.pacman, Pm.zypper, Pm.apk, Pm.yum, Pm.snap, Pm.flatpak };
        ...
        foreach (var manager in packageManagers)
        {
            ...
            {
                if (File.Exists(Path.Combine(dir, managerName)))
                {
                    return managerName; // Instantly returns e.g. "apt", "dnf", etc.
                }
            }
        }
